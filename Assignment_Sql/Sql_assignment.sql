...
SELECT lastname,POSITION('N' IN lastname)FROM list;
...
SELECT lastname,SUBSTR(lastname, 4,2)FROM list;
...
SELECT car_names.id FROM car_names RIGHT JOIN cars_data ON cars_data.id = car_names.id

UNION  

SELECT cars_data.id FROM car_names RIGHT JOIN cars_data ON cars_data.id = car_names.id;
...
SELECT c1.horsepower, c1.id, c2.id FROM cars_data c1  INNER JOIN cars_data c2 ON c1.id = c2.id AND c1.id > c2.id ORDER BY c1.horsepower;
